...
exports.selectAll = async (keyword = '', category = '', sort, page = 1, limit, isRecommended) => {
  ...
  ORDER BY ${sort === 'category' ? '"c"."name"' : `"p".${sort}`} ASC
...
exports.countAll = async (keyword = '', category = '', isRecommended) => {
  const sql = `SELECT COUNT(*) as "counts" FROM
  (SELECT
    "p"."id" AS "id",
    "p"."name" AS "name",
    array_agg(distinct "c"."name") AS "category",
    ...
    ${isRecommended ? 'AND "p"."isRecommended" = TRUE' : ''}
    GROUP BY "p"."id","t"."id"
    ${category !== '' ? `HAVING COUNT(DISTINCT "c"."name") = ${category.length}` : ''}) AS "data"`
  const values = [`%${keyword}%`]
  const { rows } = await db.query(sql, values)
...
exports.insert = async (data) => {
  const sql = `INSERT INTO "products"("name","basePrice","description","image","isRecommended") VALUES
  ($1,$2,$3,$4,$5) RETURNING *`
