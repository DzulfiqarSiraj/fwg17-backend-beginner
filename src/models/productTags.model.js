...
exports.countAll = async () => {
  const sql = 'SELECT COUNT(*) as "counts" FROM "productTags"'
  const values = []
  const { rows } = await db.query(sql, values)
...
exports.selectOneByProductIdAndTagId = async (productId, tagId) => {
  ...
  JOIN "products" "p" ON "p"."id"="pt"."productId"
  JOIN "tags" "t" ON "t"."id"="pt"."tagId"
  WHERE "pt"."productId" = $1 AND "pt"."tagId" = $2
  GROUP BY "p"."id","t"."id","pt"."id"`
...
exports.insert = async (data) => {
  const sql = `INSERT INTO "productTags"("productId","tagId") VALUES
  ($1,$2)
  RETURNING *`
